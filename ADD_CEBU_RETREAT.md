...
新しいセブ・クリマコビーチリトリートをデータベースに追加する手順です。

## 方法1: Supabase管理画面でSQLを実行（推奨）
...
```sql
-- 一時的にanonユーザーにもINSERT権限を付与（開発環境のみ）
-- 既存のポリシーがあれば削除
...
追加後、以下のURLで確認できます：
- 国際ツアーページ: `http://localhost:5173/international` (開発サーバー実行時)
- ツアー詳細ページ: `http://localhost:5173/retreat/cebu-climaco-beach-retreat`
